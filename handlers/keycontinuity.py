...
@handler(handlers,handler=config.V_KEYCONTINUITY)
class KeyContinuity(BaseHandler):
    ...
    def on_certificate(self,cert):
        ...
        algorithm = cert.get_cert_nss().subject_public_key_info.algorithm.id_str
        _id = algorithm + ' - ' + name
        exist = db.get(_id)
        if exist == None:
            ...
            if correct == False:
                ...
                logger.info("\t[-] Certificate %s has changed from \n%s--->%s\n to \n%s--->%s" % (name,before["_id"],before["hash"],algorithm, hash_t ))
                MITMNotification.notify(title=self.name,message = name)
            else:
                debug_logger.debug("\t[+] Certificate %s has not changed" % name)



#This db is shared by all the instance of Pinning. It is not necessary create an instance each time that
#we use this class.
